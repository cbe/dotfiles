...
if [[ "yes" == $(ask_yes_or_no "Symlink '~/.zshrc' to dotfiles?") ]]
then
  ln -sf ~/dotfiles/zsh/zshrc ~/.zshrc
fi

if [[ "yes" == $(ask_yes_or_no "Symlink '~/.gitconfig' to dotfiles?") ]]
then
  if [[ "yes" == $(ask_yes_or_no "Regarding '~/.gitconfig', are you at work?") ]]
  then
...
if [[ "yes" == $(ask_yes_or_no "Symlink '~/.config/nvim/init.vim' to dotfiles?") ]]
then
  ln -sf ~/dotfiles/vim/vimrc ~/.config/nvim/init.vim
fi

if [[ "yes" == $(ask_yes_or_no "Symlink '~/.config/kitty' to dotfiles?") ]]
then
  ln -sfn ~/dotfiles/kitty ~/.config/kitty
fi

if [[ "yes" == $(ask_yes_or_no "Symlink VSCodium to dotfiles?") ]]
then
  if [[ _is_linux ]]
  then
...
if [[ "yes" == $(ask_yes_or_no "Install VSCodium extensions?") ]]
then
  cat ~/dotfiles/vscodium/extensions.txt | xargs -L 1 codium --install-extension &> /dev/null
fi

if [[ "yes" == $(ask_yes_or_no "Symlink '~/.emacs.d' to dotfiles?") ]]
then
